  ...
  return (
    <div className="px-6 md:px-10 py-14 md:py-20 max-w-[1180px] mx-auto">
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-14">
        <span className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.22em] text-[#D4AF37] mb-5"><Sparkles className="w-4 h-4" /> Diretor Executivo Digital</span>
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-stretch">
        {founderOpen ? (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
            ...
            className="relative rounded-3xl p-8 border-2 border-[#D4AF37] bg-gradient-to-b from-[#D4AF37]/[0.10] to-transparent lg:-mt-4 lg:mb-4 shadow-[0_0_50px_-12px_rgba(212,175,55,0.35)] hover:-translate-y-1 transition-transform duration-300">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 inline-flex items-center gap-1.5 rounded-full bg-[#D4AF37] text-[#0B0C10] px-4 py-1.5 text-xs font-semibold shadow-lg">
            ...
            <div className="flex items-end gap-2 mt-4 mb-1">
              <span className="font-serif-lux text-5xl text-[#D4AF37]">29 €</span>
            ...
            <div className="rounded-2xl bg-[#0B0C10]/40 border border-[#D4AF37]/20 p-4 mb-6" data-testid="founder-counter">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="flex items-center gap-1.5 text-muted-foreground"><Users className="w-4 h-4 text-[#D4AF37]" /> Empresas Fundadoras</span>
            ...
            <ul className="space-y-3 mb-8">
              {FOUNDER_FEATURES.map((f) => (
                <li key={f} className="flex items-start gap-2 text-sm"><Check className="w-4 h-4 text-[#D4AF37] mt-0.5 shrink-0" />{f}</li>
              ))}
            </ul>
            <button data-testid="checkout-founder" onClick={() => checkout("founder_monthly")} disabled={loading === "founder_monthly"}
              className="w-full rounded-full bg-[#D4AF37] text-[#0B0C10] hover:bg-[#c9a431] hover:-translate-y-0.5 font-semibold py-4 transition-all inline-flex items-center justify-center gap-2">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="w-12 h-12 rounded-2xl bg-[#D4AF37]/15 flex items-center justify-center mb-4"><Flame className="w-6 h-6 text-[#D4AF37]" /></div>
      ...
      <section className="mt-20 rounded-3xl surface p-8 md:p-12 border border-[#D4AF37]/20" data-testid="why-founder">
        <div className="flex items-center gap-2 text-[#D4AF37] mb-4"><Flame className="w-5 h-5" /><span className="text-xs uppercase tracking-[0.2em]">Oportunidade rara</span></div>
