...
const color = (s) => (s >= 70 ? "#10B981" : s >= 45 ? "#3B82F6" : "#EF4444");
      ...
      <div className="grid lg:grid-cols-2 gap-10 items-center mb-14">
        <div className="surface rounded-3xl p-6 relative">
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center"><div className="font-serif-lux text-6xl text-[#3B82F6]">{data.overall}</div><div className="text-xs text-muted-foreground">/100</div></div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-6">
            {data.dimensions.map((dim, i) => (
              <button key={dim.dimension} onClick={() => setActive(i)} data-testid={`dim-btn-${i}`}
                className={`p-3 rounded-xl border text-left transition-colors ${i === active ? "border-[#3B82F6] bg-[#3B82F6]/8" : "border-border hover:bg-accent"}`}>
