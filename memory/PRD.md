...
- ✅ **Aviso por email de prazos** (2026-06): `evaluate_grant_alerts` envia email (Resend via `send_email_raw`, template `build_grant_deadline_html`) nos marcos 14/7/3/1 dias antes do prazo de cada candidatura acompanhada, uma vez por marco (`emailed_deadline_days`, re-armado ao mudar o prazo). Respeita a definição `email_grant_alerts` (default True) — toggle em Personalização (`email-grant-alert-toggle`). Testado por curl: envia no marco correto, idempotente, opt-out respeitado. Entrega real ao inbox não confirmável no preview.
- ✅ **Anexar documentos por candidatura** (2026-06): upload/lista/download/remoção de ficheiros dentro de cada candidatura acompanhada. Guardados em `db.grant_files` (base64, máx. 10 MB) + metadados em `application.files[]`. Endpoints `POST/GET/DELETE /api/grants/applications/{aid}/documents[/{fid}]` (premium, isolados por user/app). Frontend `Apoios.jsx` ApplicationCard: botão Anexar (`apoios-app-upload-{id}`), lista com transferir (`apoios-app-file-dl-*`) e remover (`apoios-app-file-del-*`). Testado por curl (upload 200, download content-type/size corretos, delete). Dados de teste limpos.
- ✅ **Exportar candidatura em PDF** (2026-06): `GET /api/grants/applications/{aid}` devolve a candidatura + detalhes do catálogo (montante, despesas, prazo oficial, verified_at) + nome da empresa. Página de relatório dedicada A4 `pages/ApoioReport.jsx` (rota `/relatorio-apoio/:aid`, fora do AppLayout, tema claro estilo `MetaReport`) com apoio/entidade, estado, prazos, montante, despesas elegíveis, passos (feito/pendente), documentos exigidos, documentos anexados, notas e disclaimer + botão "Imprimir / Guardar PDF" (`window.print`, CSS `@media print`). Botão `apoios-app-export-{id}` na candidatura abre o relatório em novo separador. Testado por curl (single-get 404 em id inválido, dados enriquecidos) + compilação frontend.
...
- ✅ **Testado ponta-a-ponta com chave real (2026-06)**: busca `grandes_obras`/Lisboa → 49 empresas reais (nome/telefone/website + email best-effort, filtrado de placeholders tipo user@domain.com/exemplo.pt), 8s via URL externa (sem 502). `update` (delta) adiciona só novas. `to-crm` criou 54 leads com score+contacto e não duplica na 2ª vez. Chave Google Places API (New) ativada pelo utilizador. Dados de teste limpos da conta admin do preview.
- ✅ **Contacto direto + seleção + contactado** (2026-06): botões por linha WhatsApp (`prosp-wa-{i}`, abre wa.me com proposta IA + telefone) e Email (`prosp-email-{i}`, abre mailto com assunto+corpo IA), ambos marcam a empresa como contactada. Checkboxes (`prosp-check-{i}`, `prosp-select-all`) → "Enviar N ao CRM" (envio seletivo via `ids`). Botão marcar contactada (`prosp-markcontacted-{i}`) + `POST /api/prospecting/contacted`; contactadas somem das buscas/lista (toggle `prosp-toggle-contacted` para as ver). Testado ponta-a-ponta.
- ⚠️ **Perf da extração de emails**: HTML limitado a 300KB antes da regex (evita bloqueio do event loop) + concorrência 40 + orçamento 25s + timeout 5s/site. Busca reparos/Cascais responde em ~13s via URL externa (antes 502 por >60s).
