...
- 📌 FASE 2 (2026-07-31, concluída): gráfico recharts de 3 linhas (verde meta / laranja ritmo atual / azul "meus ajustes"), 3 cenários (Conservador/Realista/Ambicioso — Realista por defeito, clicáveis), sliders interativos (crescimento faturação, margem, redução dívida, receitas recorrentes) com **recálculo instantâneo no cliente** (espelha `core.value_multiple`; NÃO gasta créditos LLM), badge de probabilidade ao vivo, e plano ano a ano (Ano/Faturação/Lucro/Margem/EBITDA*/Dívida/Valor/% meta/Prioridade). Validado por screenshot autenticado (valor 281k→Realista 685k, cenários e tabela coerentes). FASE 3 pendente: "Como foi calculado?" + relatório PDF.
